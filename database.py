...
class MediaOutlet(Base):
    ...
    )  # Average visit duration (mm:ss format)
    ...
    target_audience = Column(
        Text, nullable=False
    )  # JSON array - derived from demographics
...
class MLFeedbackData(Base):
    """Enterprise Feedback Data for ML Training"""
    
    __tablename__ = "ml_feedback_data"
    
    id = Column(Integer, primary_key=True, index=True)
    enterprise_id = Column(String(255), index=True, nullable=False)
    campaign_id = Column(String(255), index=True, nullable=False)
    
    # Campaign Details
    campaign_title = Column(String(500), nullable=False)
    campaign_type = Column(String(100), nullable=False)
    industry = Column(String(100), nullable=False)
    target_audience = Column(Text, nullable=False)  # JSON
    budget = Column(Float, nullable=False)
    
    # Media Performance Metrics
    total_articles_published = Column(Integer, default=0)
    total_media_reach = Column(Integer, default=0)
    click_through_rate = Column(Float, default=0.0)
    social_media_shares = Column(Integer, default=0)
    media_quality_score = Column(Float, default=0.0)  # 0-10
    
    # Business Impact Metrics
    brand_awareness_lift = Column(Float, default=0.0)  # %
    lead_generation = Column(Integer, default=0)
    sales_conversion = Column(Integer, default=0)
    roi_percentage = Column(Float, default=0.0)
    
    # Audience Engagement Metrics
    average_engagement_rate = Column(Float, default=0.0)
    comment_sentiment_score = Column(Float, default=0.0)  # -1 to 1
    ...
    # Computed Ground Truth Score (0-1)
    ground_truth_score = Column(Float, nullable=False)
    confidence_score = Column(Float, default=0.8)
    
    # Metadata
    feedback_source = Column(String(50), default="enterprise_api")
...
class MLModelVersion(Base):
    ...
    model_type = Column(String(100), default="MediaRankingLSTM")
    ...
    deployment_stage = Column(String(20), default="training")  # training, testing, staging, production
    deployment_percentage = Column(Float, default=0.0)  # % of traffic using this model
    
    # Performance Monitoring
    production_accuracy = Column(Float, default=0.0)
    production_samples = Column(Integer, default=0)
    last_evaluation = Column(DateTime)
    
    # Metadata
    created_by = Column(String(100), default="continuous_learning_manager")
    notes = Column(Text)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class MLTrainingJob(Base):
    ...
    job_type = Column(String(50), default="retraining")  # retraining, fine_tuning, validation
    ...
    memory_peak_gb = Column(Float, default=0.0)
...
class MLFeedbackDataResponse(BaseModel):
    """ML Feedback Data response model"""
...
class MLModelVersionResponse(BaseModel):
    """ML Model Version response model"""
...
class MLTrainingJobResponse(BaseModel):
    """ML Training Job response model"""
        ...
        try:
            ...
            if "cost_per_article" not in media:
                media["cost_per_article"] = estimate_cost_by_tier_and_visits(
                    media["tier"], media.get("monthly_visits")
                )
            ...
            if "target_audience" not in media:
                ...
                if category == "BUSINESS":
                    ...
                    audiences.extend(
                        ["Tech enthusiasts", "IT professionals", "Early adopters"]
                    )
                elif category == "YOUTH_ENTERTAINMENT":
                    audiences.extend(
                        ["Young adults", "Entertainment fans", "Social media users"]
                    )
            ...
            logger.error(
                f"❌ Error standardizing media data for {media.get('name', 'Unknown')}: {e}"
            )
...
def validate_media_data_structure():
        ...
        try:
            ...
            for field in required_fields:
                if field not in media or not media[field]:
                    issues.append(
                        f"Media {i+1} ({media.get('name', 'Unknown')}): Missing {field}"
                    )

            # Type validation
            if media.get("tier") not in [1, 2, 3]:
                issues.append(
                    f"Media {i+1} ({media.get('name', 'Unknown')}): Invalid tier {media.get('tier')}"
                )

            if (
                not isinstance(media.get("cost_per_article", 0), (int, float))
                or media.get("cost_per_article", 0) <= 0
            ):
                issues.append(
                    f"Media {i+1} ({media.get('name', 'Unknown')}): Invalid cost {media.get('cost_per_article')}"
                )

            # List validation
            if not isinstance(media.get("topics", []), list):
                issues.append(
                    f"Media {i+1} ({media.get('name', 'Unknown')}): topics should be list"
                )

            if not isinstance(media.get("target_audience", []), list):
                issues.append(
                    f"Media {i+1} ({media.get('name', 'Unknown')}): target_audience should be list"
                )
    ...
    if issues:
        ...
        logger.info(
            f"✅ All {len(VIETNAMESE_MEDIA_DATA)} media outlets validated successfully"
        )
...
class MediaDatabase:
    ...
    def _setup_vector_db(self):
        ...
        if not chroma_client or not embedding_model:
            logger.warning(
                "⚠️ Vector database not available - ChromaDB or embedding model missing"
            )
            return
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"✅ Vector collection '{CHROMA_COLLECTION_NAME}' created"
                    )
                except Exception as create_error:
                    logger.error(
                        f"❌ Vector collection creation failed: {create_error}"
                    )
                    self.vector_collection = None
    ...
    async def initialize_media_data(self) -> bool:
        ...
        try:
            async with self.get_db_session() as db:
                ...
                if existing_count > 0:
                    logger.info(
                        f"📊 Database already initialized with {existing_count} media outlets"
                    )
                    return True

                logger.info(
                    "🔄 Initializing comprehensive Vietnamese media database..."
                )
                ...
                logger.info(
                    f"✅ Initialized {len(VIETNAMESE_MEDIA_DATA)} Vietnamese media outlets"
                )
                return True
    ...
    async def _initialize_vector_embeddings(self, db: Session):
        ...
        try:
            ...
            for media in media_outlets:
                ...
                top_categories = (
                    json.loads(media.top_categories) if media.top_categories else []
                )
                topics = json.loads(media.topics) if media.topics else []
                audiences = (
                    json.loads(media.target_audience) if media.target_audience else []
                )
            ...
            logger.info(
                f"✅ Created vector embeddings for {len(embeddings)} media outlets"
            )
    ...
    async def search_media_by_vector(
        ...
        if not self.vector_collection or not embedding_model:
            logger.warning(
                "⚠️ Vector search not available, falling back to keyword search"
            )
        ...
        try:
            ...
            logger.debug(
                f"🔍 Vector search for '{query}' returned {len(results.get('documents', [[]])[0])} results"
            )
            return results
    ...
    async def search_media_by_category(
        ...
        try:
            async with self.get_db_session() as db:
                ...
                for media in media_outlets:
                    ...
                    try:
                        top_categories = (
                            json.loads(media.top_categories)
                            if media.top_categories
                            else []
                        )
                    ...
                    try:
                        target_audience = (
                            json.loads(media.target_audience)
                            if media.target_audience
                            else []
                        )
                    except (json.JSONDecodeError, TypeError):
                        target_audience = []
                    ...
                    result.append(
                        MediaOutletResponse(
                            ...
                            language=media.language
                            or "Vietnamese",  # Fix: Safe language access
    ...
    async def get_top_media_by_traffic(
        self, limit: int = 20
    ) -> List[MediaOutletResponse]:
    ...
    async def save_agent_recommendations(
        self, request_id: int, recommendations: List[Dict[str, Any]], agent_name: str
    ) -> bool:
        ...
        try:
            async with self.get_db_session() as db:
                ...
                db.commit()
                logger.info(
                    f"✅ Saved {len(recommendations)} recommendations from {agent_name}"
                )
                return True
    ...
    async def get_media_statistics(self) -> Dict[str, Any]:
        ...
        try:
            async with self.get_db_session() as db:
                ...
                total_outlets = (
                    db.query(MediaOutlet).filter(MediaOutlet.is_active == True).count()
                )

                # Category breakdown
                categories = (
                    db.query(MediaOutlet.category, func.count(MediaOutlet.id))
                    .filter(MediaOutlet.is_active == True)
                ...
                # Tier breakdown
                tiers = (
                    db.query(MediaOutlet.tier, func.count(MediaOutlet.id))
                    .filter(MediaOutlet.is_active == True)
                ...
                # Top traffic outlets
                top_traffic = (
                    db.query(MediaOutlet.name, MediaOutlet.monthly_visits)
                    .filter(MediaOutlet.is_active == True)
                ...
                avg_costs = (
                    db.query(MediaOutlet.tier, func.avg(MediaOutlet.cost_per_article))
                    .filter(MediaOutlet.is_active == True)
                    .group_by(MediaOutlet.tier)
                    .all()
                )
                ...
                return {
                    ...
                    "top_traffic": [
                        {"name": name, "monthly_visits": visits}
                        for name, visits in top_traffic
                    ],
                    "average_costs_by_tier": {
                        tier: float(avg_cost) for tier, avg_cost in avg_costs
                    },
                    "total_monthly_visits": sum(
                        [visits for _, visits in top_traffic if visits]
                    ),
                }
            ...
            async with self.get_db_session() as db:
                ...
                if category:
                    query_obj = query_obj.filter(
                        MediaOutlet.category == category.upper()
                    )
                ...
                if min_visits:
                    query_obj = query_obj.filter(
                        MediaOutlet.monthly_visits >= min_visits
                    )

                if max_cost:
                    query_obj = query_obj.filter(
                        MediaOutlet.cost_per_article <= max_cost
                    )
                ...
                media_outlets = (
                    query_obj.order_by(MediaOutlet.monthly_visits.desc())
                    .limit(limit)
                    .all()
                )
...
async def init_database():
    ...
    try:
        ...
        if success:
            ...
            logger.info(
                f"   • Categories: {', '.join(stats.get('categories', {}).keys())}"
            )
...
async def get_media_categories() -> List[str]:
    ...
    try:
        async with media_db.get_db_session() as db:
            categories = (
                db.query(MediaOutlet.category.distinct())
                .filter(MediaOutlet.is_active == True)
                .all()
            )
...
async def get_recommended_media_for_budget(
    budget: float, limit: int = 10
) -> List[MediaOutletResponse]:
    ...
    try:
        ...
        return await media_db.search_media_advanced(
            max_cost=max_cost_per_article, limit=limit
        )
    except Exception as e:
...
def calculate_audience_fit_score(
    media: MediaOutletResponse, target_demographics: Dict
) -> float:
    ...
    if media.monthly_visits:
        ...
        traffic_score = min(
            1.0, media.monthly_visits / 50000000
        )  # 50M visits = max score
...
async def test_database():
    ...
    try:
        ...
        # Test vector search
        results = await media_db.search_media_by_vector(
            "technology startup business fintech"
        )
        logger.info(
            f"🔍 Vector search returned {len(results.get('documents', [[]])[0])} results"
        )
        ...
        premium_media = await media_db.search_media_advanced(
            tier=1, min_visits=1000000, max_cost=10000000, limit=5
        )
...
def validate_media_data():
    ...
    for i, media in enumerate(VIETNAMESE_MEDIA_DATA):
        ...
        if not media.get("website", "").startswith("http"):
            issues.append(
                f"Media {i+1} ({media.get('name', 'Unknown')}): Invalid website URL"
            )
