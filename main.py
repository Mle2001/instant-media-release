...
class Settings(BaseSettings):
    ...
    api_description: str = (
        "AI-powered press release automation với Payment, Distribution, Content Generation"
    )
...
async def health_check():
    ...
    if "ERROR" in db_status or "ERROR" in agent_status:
        ...
        "WARNING" in status
        for status in [doc_status, payment_status, distribution_status, content_status]
    ):
    ...
    try:
        ...
        if hasattr(context, "media_recommendations"):
            media_plan["selected_media"] = [
                ...
                for rec in (context.media_recommendations or [])[
                    : package_info["media_count"]
                ]
            ]
...
async def generate_content(request: ContentGenerateRequest):
    ...
    try:
        if not content_agent:
            raise HTTPException(
                status_code=503, detail="Content generation service not available"
            )
        ...
        order_id = (
            f"content-{request.session_id}-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
        )
...
async def enhance_content(content_id: str, enhancement_types: List[str]):
    ...
    try:
        ...
        enhanced_content = content_agent.enhance_existing_content(
            content_id, enhancement_types
        )
...
async def create_distribution_campaign(request: DistributionCreateRequest):
    ...
    try:
        if not distribution_agent:
            raise HTTPException(
                status_code=503, detail="Distribution service not available"
            )

        # Get press release content
        if not content_agent:
            raise HTTPException(
                status_code=503, detail="Content service not available for distribution"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Distribution campaign creation failed"
        )
...
async def get_distribution_analytics(order_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get distribution analytics"
        )
...
async def generate_performance_report(request: PerformanceReportRequest):
    ...
    try:
        ...
        if content_agent:
            try:
                ...
                content_analytics = {
                    ...
                    "readability_score": content_data.get(
        ...
        if distribution_agent:
            try:
                dist_data = distribution_agent.get_distribution_analytics(
                    request.order_id
                )
                distribution_analytics = {
        ...
        if distribution_analytics.get("response_rate"):
            scores.append(
                distribution_analytics["response_rate"] * 2
            )  # Weight response higher
        ...
        raise HTTPException(
            status_code=500, detail="Performance report generation failed"
        )
                        ...
                        status_message = {
                            ...
                            "data": {
                                ...
                                "enhanced_features": {
                                    ...
                                    "distribution_available": distribution_agent
                                    is not None,
                                    "content_generation_available": content_agent
                                    is not None,
                                },
                            },
                        }
