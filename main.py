...
class Settings(BaseSettings):
    ...
    # OpenAI Configuration (add these fields to match .env)
    openai_api_key: str = Field(..., alias="OPENAI_API_KEY")
    openai_model: str = Field("gpt-4o-mini", alias="OPENAI_MODEL")
    openai_temperature: float = Field(0.3, alias="OPENAI_TEMPERATURE")
...
async def lifespan(app: FastAPI):
    ...
    openai_key = os.getenv(
        "OPENAI_API_KEY",
        "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",
    )
    if openai_key:
...
async def trigger_workflow_background(session_id: str, progress_callback):
    ...
    try:
        ...
        if hasattr(response, "data") and response.data:
            ...
            logger.info(
                f"📊 Workflow data keys: {list(workflow_data.keys()) if isinstance(workflow_data, dict) else 'Not dict'}"
            )
        ...
        if session_id in active_websockets:
            try:
                ...
                if final_data:
                    logger.info(
                        f"   - Workflow data keys: {list(final_data.keys()) if isinstance(final_data, dict) else 'Not dict'}"
                    )
                else:
