...
CHATBOT_QUESTIONS = [
    {
        ...
        "type": "choice",
        "options": [
            "Tôi có bản tiếng Việt",
            "Tôi có bản tiếng Anh",
            "Tôi chưa có, cần hỗ trợ viết",
        ],
    },
    {
        "id": 2,
...
async def start_agent_processing(
    ...
    try:
        ...
        return ProcessingResponse(
            success=False, session_id=request.session_id, error=str(e)
        )


async def process_with_agents_background(
    session_id: str, user_input: str, budget: float
):
    ...
    try:
        ...
        logger.info(
            f"✅ Background processing completed: {session_id} ({processing_time:.2f}s)"
        )
        ...
        if session_id in active_websockets:
            try:
                await active_websockets[session_id].send_text(
                    json.dumps({"type": "processing_error", "error": str(e)})
                )
            except:
                pass
    ...
    try:
        while True:
            ...
            if message.get("type") == "ping":
                ...
                await websocket.send_text(
                    json.dumps({"type": "status_update", "status": status})
                )
