...
from database import media_db, get_database, UserRequest # Import UserRequest
from document_processor import get_document_processor
from services import StrategyOptimizerService # NEW: Import the optimizer service
...
class ConversationalMediaReleaseAgents:
    ...
    async def _execute_complete_workflow(self, context: ConversationContext) -> Dict[str, Any]:
        ...
        try:
            ...
            context.content_analysis = content_analysis.model_dump()
            workflow_results["content_analysis"] = context.content_analysis

            # --- NEW: Few-Shot Learning Integration ---
            await update_progress(context, "Learning from Past", "🧠 Tham khảo các chiến dịch thành công...")
            optimizer_service = StrategyOptimizerService(db)
            few_shot_examples = await optimizer_service.get_few_shot_examples(context)
            ...
            if document_analysis:
                context.document_analysis = document_analysis.model_dump()
                workflow_results["document_analysis"] = context.document_analysis
            
            # Step 3: Media Matching (UPDATED with few_shot_examples)
            await update_progress(context, "Media Matching", "🎯 Tìm kiếm báo chí phù hợp...")
            media_recommendations = await self._find_matching_media_with_context(context, content_analysis, document_analysis, few_shot_examples)
            workflow_results["media_recommendations"] = [rec.model_dump() for rec in media_recommendations]
            
            # Step 4: Pricing Optimization (UPDATED with few_shot_examples)
            await update_progress(context, "Pricing Optimization", "💰 Tối ưu hóa gói dịch vụ...")
            pricing_analysis = await self._optimize_pricing_with_context(context, media_recommendations, few_shot_examples)
        ...
        try:
            ...
            for i, candidate in enumerate(media_candidates[:10]):  # Process top 10
                try:
                    ...
                    USER PREFERENCES: {json.dumps(context.preferences, ensure_ascii=False)}
                    {document_context}
                    PREVIOUS SUCCESS EXAMPLES: {few_shot_examples}
