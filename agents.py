...
# ML system availability flag - components loaded lazily to avoid circular imports
ML_SYSTEM_AVAILABLE = False

def _load_ml_system():
    """Lazily load ML ranking system components to avoid circular imports"""
    global ML_SYSTEM_AVAILABLE
    try:
        ...
        logger.info("✅ ML ranking system loaded successfully")
        return EnhancedMediaRankingAgent, UniversalStructureExtractor, ContinuousLearningManager
    except ImportError as e:
        logger.warning(f"⚠️ ML ranking system not available: {e}")
        ML_SYSTEM_AVAILABLE = False
...
GROQ_ORCHESTRATOR_MODEL = os.getenv(
...
class ConversationContext:
    ...
    # Flag to detect if input was from a quick button or real user text
...
class ConversationResponse(BaseModel):
    ...
    options: List[str] = Field(
        description="Available action options for user at current stage", default=[]
    )

    progress: Optional[ProgressInfo] = Field(
        description="Progress information for ongoing team processing", default=None
    )

    data: Optional[DataField] = Field(
        description="Additional structured data for frontend", default=None
    )

    requires_input: bool = Field(
        description="Whether system is waiting for user input", default=True
    )
...
class DocumentAnalysis(BaseModel):
    ...
    relevant_topics: List[str] = Field(
        description="Topics relevant to media release (max 5)"
    )
    target_audience_insights: List[str] = Field(
        description="Audience insights from document (max 4)"
    )
    ...
    document_quality: str = Field(
        description="High/Medium/Low - content richness assessment"
    )
    confidence_score: float = Field(description="Analysis confidence 0-1", ge=0, le=1)
...
def parse_agent_response(result: Any, model_class: type, fallback_data: Dict = None):
    ...
    try:
        ...
        logger.debug(f"Original result causing error: {result}")
        if fallback_data:
            logger.warning(f"🔄 Using fallback data for {model_class.__name__}")
            return model_class(**fallback_data)
        raise ValueError(
            f"Failed to parse {model_class.__name__} from {type(result)}: {e}"
        )
...
class EnhancedMediaReleaseTeamSystem:
    ...
    def __init__(self):
        ...
        if not OPENAI_API_KEY and not GROQ_API_KEY:
            raise ValueError(
                "❌ Either OPENAI_API_KEY or GROQ_API_KEY environment variable is required"
            )
        ...
        if OPENAI_API_KEY:
            ...
            logger.info(
                f"✅ Using OpenAI models - Orchestrator: {OPENAI_ORCHESTRATOR_MODEL}, Workers: {OPENAI_WORKER_MODEL}"
            )
        elif GROQ_API_KEY:
            ...
            logger.info(
                f"✅ Using Groq models - Orchestrator: {GROQ_ORCHESTRATOR_MODEL}, Workers: {GROQ_WORKER_MODEL}"
            )

        # Team memory storage
        self.storage = SqliteAgentStorage(
            table_name="team_agents", db_file="./team_memory.db"
        )
    ...
    def _initialize_core_teams(self):
        ...
        self.conversation_agent = Agent(
            ...
            instructions=[
                ...
                "IMPORTANT: All messages must be written in natural Vietnamese, no English.",
        ...
        # Media Matching Specialist (Enhanced with ML)
        # Try to load ML system lazily
        EnhancedMediaRankingAgent, _, _ = _load_ml_system()
        
        if ML_SYSTEM_AVAILABLE and EnhancedMediaRankingAgent:
            try:
                self.media_specialist = EnhancedMediaRankingAgent()
                logger.info("✅ Enhanced ML Media Specialist initialized")
            except Exception as e:
                logger.warning(f"⚠️ Failed to initialize ML Media Specialist: {e}")
        ...
        self.report_specialist = Agent(
            ...
            model=self.worker_llm,
            instructions=[
                "IMPORTANT: All text content must be in Vietnamese (tiếng Việt)",
                "Tạo báo cáo chiến lược cấp điều hành kết hợp thông tin cuộc trò chuyện",
                "Chuyển đổi khuyến nghị kỹ thuật thành thông tin kinh doanh",
                "Trình bày chiến lược phù hợp với mức độ phức tạp cho người dùng",
                "Xác định chỉ số thành công phù hợp với mục tiêu đã nêu của người dùng",
                "Cung cấp lộ trình triển khai có thể thực hiện được",
                "Giải quyết rủi ro đồng thời nổi bật lợi thế cạnh tranh",
                "CRITICAL: All content fields must be in Vietnamese language only",
                "Return ONLY valid JSON in ExecutiveReport format with Vietnamese content",
            ],
    ...
    async def start_conversation(self, session_id: str) -> ConversationResponse:
        ...
        try:
            ...
            context.options = response.options or []
            context.suggestions = response.suggestions or []
    ...
    async def continue_conversation(
        ...
        # Check if it's a quick button
        recent_buttons = (context.options or []) + (context.suggestions or [])
        context.is_button_triggered = user_message.strip() in recent_buttons

        if context.is_button_triggered:
            return await self.handle_quick_button(session_id, user_message)
        else:
            return await self.handle_user_text(
                session_id, user_message, progress_callback
            )

    async def handle_quick_button(
        self, session_id: str, user_message: str
    ) -> ConversationResponse:
        """Handle quick button input by asking a follow-up"""
        context = self.active_conversations.get(session_id)
        if not context:
            return await self.start_conversation(session_id)
        ...
        # Prompt để gợi mở follow-up
        prompt = f"""
        The user has just clicked a quick suggestion button: "{user_message}". This indicates an *intention*, not complete information.

        Please respond by:
        - NOT thanking the user
        - NOT confirming understanding
        - Asking an OPEN-ENDED follow-up question to elicit more specific input

        Examples:
        - If the button was 'Ngân sách dự kiến của bạn' → ask: 'Bạn dự kiến ngân sách khoảng bao nhiêu cho dự án lần này?'
        - If the button was 'Mục tiêu truyền thông' → ask: 'Bạn muốn đạt được điều gì thông qua chiến dịch truyền thông này?'

        RESPONSE FORMAT: Return a valid JSON in the format of ConversationResponse, using Vietnamese only for the message.
        """

        fallback = {
            "message": "Bạn có thể chia sẻ thêm thông tin cụ thể hơn được không?",
        ...
        result = await self.conversation_agent.arun(prompt)
        response = parse_agent_response(result, ConversationResponse, fallback)
        ...
        context.is_button_triggered = False
        context.options = response.options or []
        context.suggestions = response.suggestions or []
    ...
    async def handle_user_text(
        ...
        """Handle user text input with full context update and analysis"""
        context = self.active_conversations.get(session_id)
        if not context:
            return await self.start_conversation(session_id)
        ...
        if not user_message.strip():
            return ConversationResponse(
                message="Bạn có thể nhập thêm nội dung cụ thể để tôi hỗ trợ tốt hơn?",
                state=context.state.value,
                phase=context.phase.value,
                requires_input=True,
                can_proceed=False,
        ...
        IMPORTANT:
        - ALWAYS respond to the user in Vietnamese, even if the system prompt is in English.
        ...
        response = parse_agent_response(result, ConversationResponse, fallback_response)
    ...
    async def trigger_workflow(
        ...
        try:
            ...
            if (
                progress_callback
                and progress_callback not in context.progress_callbacks
            ):
                context.progress_callbacks.append(progress_callback)
            ...
            self.analysis_team.session_state = {
                ...
                "user_context": {
                    ...
                    "conversation_summary": self._extract_key_info_from_conversation(
                        context
                    ),
                },
                "workflow_results": {},
            ...
            await update_progress(
                context, "Team Initialization", "🚀 Khởi tạo đội ngũ chuyên gia AI..."
            )
    ...
    Đội ngũ chuyên gia đã phân tích toàn diện và xác minh chất lượng! 👇"""
            ...
            try:
                ...
                logger.warning(
                    f"Response parsing failed, using fallback: {parse_error}"
                )
            ...
            context.media_recommendations = workflow_result.get(
            ...
            logger.info(
                f"✅ Enhanced team workflow completed for session: {session_id}"
            )
            return response
    ...
    async def modify_plan(
        ...
        try:
            ...
            await update_progress(
                context,
                "Plan Modification",
                "🔄 Đội ngũ chuyên gia đang phân tích yêu cầu thay đổi...",
            )
            ...
            modification_analysis = parse_agent_response(
                result,
                PlanModification,
                {
            ...
            response = parse_agent_response(
                ...
                ConversationResponse,
                {
                    ...
                    "data": {
                        "modification_analysis": modification_analysis.model_dump()
                    },
            ...
            logger.info(
                f"🔄 Plan modification analyzed by specialist for session: {session_id}"
            )
            return response
    ...
    async def _execute_enhanced_team_workflow(
        ...
        try:
            ...
            await update_progress(
                context, "Team Initialization", "⚙️ Khởi tạo và phân công nhiệm vụ..."
            )
            ...
            await update_progress(
                context, "Parallel Analysis", "🔄 Chạy song song 5 chuyên gia AI..."
            )
            ...
            content_task = self._run_content_analysis_with_context(
            ...
            if PARALLEL_PROCESSING:
                ...
                initial_results = await asyncio.gather(
                    *[task for _, task in parallel_tasks[:2]], return_exceptions=True
                )

                # Process initial results
                for i, (task_name, result) in enumerate(
                    zip(["content_analysis", "document_analysis"], initial_results)
                ):
                    if not isinstance(result, Exception):
                        workflow_results[task_name] = (
                            result.model_dump()
                ...
                await update_progress(
                    ...
                    "🎯 Phân tích báo chí và chiến lược...",
                )
                ...
                workflow_results["media_recommendations"] = [
                    rec.model_dump() for rec in media_recommendations
                ]
                ...
                dependent_results = await asyncio.gather(
                    pricing_task, report_task, return_exceptions=True
                )

                # Process dependent results
                for task_name, result in zip(
                    ["pricing_analysis", "executive_report"], dependent_results
                ):
                    if not isinstance(result, Exception):
                        workflow_results[task_name] = (
                            result.model_dump()
                ...
                content_analysis = await self._run_content_analysis_with_context(
                    context, shared_context
                )
                workflow_results["content_analysis"] = content_analysis.model_dump()

                document_analysis = await self._run_document_analysis_with_context(
                    context, shared_context
                )
            ...
            await update_progress(
                context,
                "Quality Assurance",
                "🔍 Kiểm chứng chất lượng và xác minh nguồn...",
            )
            ...
            await update_progress(
                context, "Synthesis", "📋 Tổng hợp kết quả từ đội ngũ chuyên gia..."
            )
            ...
            try:
                ...
                if pricing_data:
                    recommended_package = pricing_data.get(
                        "recommended_package", "Standard"
                    )
            ...
            workflow_results.update(
                {
                    ...
                    "team_performance": {
                        ...
                        "tasks_completed": len(
                            [
                                k
                                for k in workflow_results.keys()
                                if not k.startswith("team_")
                            ]
                        ),
                    ...
                    "summary": {
                        ...
                        "timeline": timeline,
                        "budget_utilization": (
                            min(1.0, total_cost / max(context.budget, 1))
                            if context.budget > 0
                            else 0.8
                        ),
                        ...
                        "documents_analyzed": bool(
                            workflow_results.get("document_analysis")
                        ),
                    ...
                    "metadata": {
                        ...
                        "orchestrator_model": (
                            ...
                            if OPENAI_API_KEY
                            else GROQ_ORCHESTRATOR_MODEL
                        ),
                        "worker_model": (
                            OPENAI_WORKER_MODEL if OPENAI_API_KEY else GROQ_WORKER_MODEL
                        ),
                    },
                }
            )
            ...
            await update_progress(
                context, "Completed", "✅ Đội ngũ chuyên gia hoàn thành phân tích!"
            )

            logger.info(
                f"✅ Enhanced team workflow completed for session: {context.session_id}"
            )
    ...
    async def _run_content_analysis_with_context(
        self, context: ConversationContext, shared_context: Dict
    ) -> ContentAnalysis:
        ...
        try:
            ...
            fallback_data = {
                ...
                "primary_topics": shared_context["requirements"].get(
                    "topics", ["Business", "Technology"]
                ),
                "target_audiences": shared_context["requirements"].get(
                    "audiences", ["General Public"]
                ),
                "keywords": [
                    "press release",
                    shared_context["requirements"].get("industry", "business"),
                ],
                "content_tone": shared_context["preferences"].get(
                    "tone", "Professional"
                ),
                "urgency_level": shared_context["requirements"].get(
                    "urgency", "Medium"
                ),
                "industry_sector": shared_context["requirements"].get(
                    "industry", "General"
                ),
                "press_release_type": shared_context["requirements"].get(
                    "type", "Other"
                ),
                "geographic_scope": shared_context["requirements"].get(
                    "scope", "National"
                ),
                "confidence_score": 0.8,
            }

            content_analysis = parse_agent_response(
                result, ContentAnalysis, fallback_data
            )
            logger.info(
                f"✅ ContentSpecialist completed - Confidence: {content_analysis.confidence_score:.2f}"
            )
    ...
    async def _run_document_analysis_with_context(
        self, context: ConversationContext, shared_context: Dict
        ...
        try:
            ...
            search_query = (
                " ".join(search_terms)
                if search_terms
                else "business project information"
            )

            search_results = await doc_processor.search_user_documents(
                query=search_query, session_id=context.session_id, limit=10
            )
            ...
            fallback_data = {
                ...
                "relevant_topics": shared_context["requirements"].get(
                    "topics", ["Business"]
                ),
                "target_audience_insights": shared_context["requirements"].get(
            ...
            document_analysis = parse_agent_response(
                result, DocumentAnalysis, fallback_data
            )
    ...
    async def _run_media_matching_with_context(
        ...
        """Execute media matching specialist with enhanced deduplication"""
        try:
...
<<<<<<< HEAD
            ...
            # Enhanced vector search with deduplication tracking
            vector_results = await media_db.search_media_by_vector(search_query, limit=20)  # Increased limit
            
            # ============= ENHANCED DEDUPLICATION LOGIC =============
            
            media_candidates = []
            seen_media_ids = set()  # Track unique IDs
            seen_media_names = set()  # Track unique names (case-insensitive)
            
            # Process vector search results with deduplication
            if vector_results and "metadatas" in vector_results and vector_results["metadatas"]:
                logger.info(f"🔍 Processing {len(vector_results['metadatas'][0])} vector search results")
...
>>>>>>> d0b2578 (new version)
                    if "media_id" in metadata:
                        ...
                        if media_id in seen_media_ids:
                            logger.debug(f"⚠️ Skipping duplicate media_id: {media_id}")
                            continue
...
<<<<<<< HEAD
                            ...
                            normalized_name = media.name.lower().strip()
                            
                            # Skip if already seen this name (fuzzy match)
                            if self._is_duplicate_media_name(normalized_name, seen_media_names):
                                logger.debug(f"⚠️ Skipping duplicate media name: {media.name}")
                                continue
                            
                            # Add to tracking sets
                            seen_media_ids.add(media_id)
                            seen_media_names.add(normalized_name)
            ...
            logger.info(f"✅ Vector search: {len(media_candidates)} unique candidates")
            
            # Fallback to category-based search with enhanced deduplication
            if len(media_candidates) < 10:
                logger.info("🔄 Running category-based fallback search...")
                relevant_categories = self._determine_media_categories(content_analysis, context)
                
                for category in relevant_categories:
                    category_media = await media_db.search_media_by_category(category, limit=8)
                    category_added = 0
                    
                    for media in category_media:
                        # Enhanced duplicate checking
                        if media.id in seen_media_ids:
                            logger.debug(f"⚠️ Category search: Skipping duplicate ID {media.id}")
                            continue
                        
                        normalized_name = media.name.lower().strip()
                        if self._is_duplicate_media_name(normalized_name, seen_media_names):
                            logger.debug(f"⚠️ Category search: Skipping duplicate name {media.name}")
                            continue
                        ...
                        media_candidates.append({
                            ...
                            "success_rate": getattr(media, 'success_rate', 0.8) or 0.8,
                            "response_time": getattr(media, 'response_time_hours', 24) or 24,
                            "relevance_score": 0.6,  # Lower score for category matches
                            "source": "category_search"
                        })
                        
                        category_added += 1
                        if category_added >= 3:  # Limit per category
                            break
            
            logger.info(f"🎯 Total unique candidates after deduplication: {len(media_candidates)}")
            
            # ============= ENHANCED BATCH PROCESSING =============
            
            recommendations = []
            batch_size = 4  # Smaller batches for better control
            processed_ids = set()  # Additional safety check
            
            # Sort candidates by relevance score first
            media_candidates.sort(key=lambda x: (x["relevance_score"], -x["tier"], x["cost"]), reverse=True)
            
            for i in range(0, min(len(media_candidates), 12), batch_size):  # Process top 12 candidates
...
=======
                            media_candidates.append(
                                {
                                    ...
                                    "category": media.category,
                                    "tier": media.tier,
                                    "cost": getattr(media, "cost_per_article", 1000000)
                                    or 1000000,
                                    "monthly_visits": getattr(
                                        media, "monthly_visits", 1000000
                                    )
                                    or 1000000,
                                    "top_categories": getattr(
                                        media, "top_categories", []
                                    )
                                    or [],
                                    "topics": getattr(media, "topics", []) or [],
                                    "audiences": getattr(media, "target_audience", [])
                                    or [],
                                    "language": getattr(media, "language", "Vietnamese")
                                    or "Vietnamese",
                                    "success_rate": getattr(media, "success_rate", 0.8)
                                    or 0.8,
                                    "response_time": getattr(
                                        media, "response_time_hours", 24
                                    )
                                    or 24,
                                }
                            )
            ...
            if len(media_candidates) < 10:
                relevant_categories = self._determine_media_categories(
                    content_analysis, context
                )
                for category in relevant_categories:
                    category_media = await media_db.search_media_by_category(
                        category, limit=5
                    )
                    for media in category_media:
            ...
            logger.info(
                f"🎯 MediaSpecialist found {len(media_candidates)} candidate outlets"
            )
...
>>>>>>> d0b2578 (new version)
                ...
                for candidate in batch:
                    ...
                    if candidate["id"] in processed_ids:
                        logger.warning(f"⚠️ Final check: Skipping duplicate {candidate['name']} (ID: {candidate['id']})")
                        continue
                    ...
                    RELEVANCE SCORE: {candidate['relevance_score']:.3f}
                    SEARCH SOURCE: {candidate['source']}
                    
                    Evaluate based on conversation insights, budget constraints, and strategic fit.
                    Consider the relevance score from our matching algorithm.
...
<<<<<<< HEAD
                ...
                if not batch_tasks:  # Skip empty batches
                    continue
...
<<<<<<< HEAD
                                "matching_score": min(0.95, candidate["relevance_score"] + 0.1),  # Boost with relevance
                                "reasoning": f"Good fit for {content_analysis.industry_sector} in {candidate.get('category', 'general')} category (relevance: {candidate['relevance_score']:.2f})",
                                ...
                                "topic_overlap": min(0.9, candidate["relevance_score"]),
                                "audience_fit": 0.7
                            }
                            
                            recommendation = parse_agent_response(result, MediaRecommendation, fallback_data)
                            
                            # Final validation - ensure no duplicate IDs in recommendations
                            if not any(rec.media_outlet_id == recommendation.media_outlet_id for rec in recommendations):
                                recommendations.append(recommendation)
                            else:
                                logger.warning(f"⚠️ Final validation: Blocked duplicate recommendation for {recommendation.media_name}")
                                
                        else:
                            logger.warning(f"❌ Processing failed for {candidate['name']}: {result}")
            ...
            # Sort by matching score (which now includes relevance)
            recommendations.sort(key=lambda x: (x.matching_score, -x.tier), reverse=True)
            
            # Apply budget filtering with deduplication safety check
            budget_filtered = []
            cumulative_cost = 0
...
=======
                                "matching_score": min(
                                    ...
                                    0.6
                                    + (
                                        candidate.get("monthly_visits", 1000000)
                                        / 100000000
                                    ),
                                ),
                                "reasoning": f"Good fit for {content_analysis.industry_sector} in {candidate.get('category', 'general')} category",
                                "estimated_reach": candidate.get(
                                    "monthly_visits", 1000000
                                ),
                                "cost_vnd": candidate.get("cost", 1000000),
                                "tier": candidate.get("tier", 2),
                                "language_match": candidate.get(
                                    "language", "Vietnamese"
                                )
                                == content_analysis.language
                                or content_analysis.language == "Both",
                                "topic_overlap": 0.7,
                                "audience_fit": 0.7,
                            }

                            recommendation = parse_agent_response(
                                result, MediaRecommendation, fallback_data
                            )
                            recommendations.append(recommendation)
                        else:
                            ...
                            fallback_recommendation = MediaRecommendation(
                                ...
                                media_name=candidate["name"],
                                matching_score=0.7,
                                reasoning=f"Phù hợp cho lĩnh vực {content_analysis.industry_sector}",
                                estimated_reach=candidate.get(
                                    "monthly_visits", 1000000
                                ),
                                cost_vnd=candidate.get("cost", 1000000),
                        ...
                        logger.warning(
                            f"Failed to process media candidate {candidate['name']}: {e}"
                        )
            ...
            cumulative_cost = 0
            budget_limit = (
                shared_context["budget"] * 0.8
                if shared_context["budget"] > 0
                else float("inf")
            )
...
>>>>>>> d0b2578 (new version)
            for rec in recommendations:
                ...
                if rec.media_outlet_id in final_check_ids:
                    logger.error(f"🚨 CRITICAL: Duplicate detected in final filtering: {rec.media_name} (ID: {rec.media_outlet_id})")
                    continue
                    
                final_check_ids.add(rec.media_outlet_id)
...
<<<<<<< HEAD
                ...
                if len(budget_filtered) >= 8:  # Limit to top 8
                    break
            
            # ============= LOGGING AND VALIDATION =============
            
            logger.info(f"✅ MediaSpecialist completed - {len(budget_filtered)} unique recommendations")
            logger.info(f"📊 Final recommendations:")
            
            for i, rec in enumerate(budget_filtered, 1):
                logger.info(f"  {i}. {rec.media_name} (ID: {rec.media_outlet_id}) - Score: {rec.matching_score:.3f} - Cost: {rec.cost_vnd:,.0f} VND")
            
            # Validation: Check for any remaining duplicates
            final_ids = [rec.media_outlet_id for rec in budget_filtered]
            if len(final_ids) != len(set(final_ids)):
                logger.error(f"🚨 CRITICAL ERROR: Duplicates still exist in final results!")
                logger.error(f"IDs: {final_ids}")
                logger.error(f"Unique IDs: {list(set(final_ids))}")
                
                # Emergency deduplication
                seen_final = set()
                emergency_filtered = []
                for rec in budget_filtered:
                    if rec.media_outlet_id not in seen_final:
                        seen_final.add(rec.media_outlet_id)
                        emergency_filtered.append(rec)
                budget_filtered = emergency_filtered
                logger.warning(f"🔧 Emergency deduplication applied: {len(budget_filtered)} final recommendations")
    ...
    def _is_duplicate_media_name(self, name: str, seen_names: set) -> bool:
        """Check if media name is duplicate with fuzzy matching"""
        ...
        for seen_name in seen_names:
            # Check if names are very similar (accounting for Vietnamese naming patterns)
            if self._names_are_similar(name, seen_name):
                return True
        
        return False

    def _names_are_similar(self, name1: str, name2: str, threshold: float = 0.9) -> bool:
        """Check if two media names are similar enough to be considered duplicates"""
        # Remove common Vietnamese media suffixes/prefixes
        def normalize_media_name(name):
            name = name.lower().strip()
            # Remove common prefixes
            prefixes = ['báo ', 'tờ ', 'tạp chí ', 'website ', 'cổng thông tin ']
            for prefix in prefixes:
                if name.startswith(prefix):
                    name = name[len(prefix):]
            
            # Remove common suffixes
            suffixes = [' online', ' điện tử', ' vietnam', ' vn', '.vn', '.com']
        ...
        norm1 = normalize_media_name(name1)
        norm2 = normalize_media_name(name2)
        
        # If normalized names are identical
        if norm1 == norm2:
            return True
        
        # Check if one is substring of another (for cases like "VnExpress" vs "VnExpress.net")
...
=======
            ...
            logger.info(
                f"✅ MediaSpecialist completed - {len(budget_filtered)} recommendations within budget"
            )
...
>>>>>>> d0b2578 (new version)
    async def _run_pricing_optimization_with_context(
        ...
        try:
            total_media_cost = (
                sum(rec.cost_vnd for rec in recommendations) if recommendations else 0
            )
            safe_budget = max(
                shared_context["budget"] if shared_context["budget"] > 0 else 25000000,
                1000000,
            )
            ...
            budget_utilization = (
                min(1.0, total_cost / safe_budget) if safe_budget > 0 else 0.0
            )
            ...
            fallback_data = {
                ...
                "media_costs_vnd": min(
                    total_media_cost, max(0, safe_budget - package_price)
                ),
                ...
                "alternative_packages": [
                    pkg for pkg in PACKAGE_PRICES.keys() if pkg != recommended_package
                ],
            }

            pricing_analysis = parse_agent_response(
                result, PricingAnalysis, fallback_data
            )
            logger.info(
                f"✅ PricingSpecialist completed - Package: {pricing_analysis.recommended_package}"
            )
    ...
    async def _run_report_generation_with_context(
        ...
        try:
            ...
            executive_report = parse_agent_response(
                result, ExecutiveReport, fallback_data
            )
    ...
    async def _run_quality_assurance(
        self, context: ConversationContext, workflow_results: Dict
    ) -> Dict:
        ...
        try:
            ...
            if hasattr(result, "content"):
                ...
                try:
                    ...
                    if (
                        isinstance(qa_content, str)
                        and "quality_score" in qa_content.lower()
                    ):
                        # Extract quality indicators from text
                        if (
                            "high quality" in qa_content.lower()
                            or "verified" in qa_content.lower()
                        ):
            ...
            logger.info(
                f"✅ QA Team completed - Quality Score: {qa_result['quality_score']:.2f}"
            )
            return qa_result
    ...
    def _extract_key_info_from_conversation(self, context: ConversationContext) -> str:
        ...
        if context.requirements:
            ...
            if context.requirements.get("target_audience"):
                key_info.append(
                    f"Target Audience: {', '.join(context.requirements['target_audience'])}"
                )
            if context.requirements.get("objectives"):
                key_info.append(
                    f"Objectives: {', '.join(context.requirements['objectives'])}"
                )
        ...
        if context.preferences:
            if context.preferences.get("timeline"):
                key_info.append(
                    f"Preferred Timeline: {context.preferences['timeline']}"
                )
            if context.preferences.get("media_types"):
                key_info.append(
                    f"Preferred Media: {', '.join(context.preferences['media_types'])}"
                )

        # Extract from conversation patterns
        user_messages = [
            msg for msg in context.conversation_history if msg.get("role") == "user"
        ]
        if user_messages:
            recent_topics = []
        ...
        return (
            "; ".join(key_info) if key_info else "Team ready for comprehensive analysis"
        )
    ...
    async def _update_context_from_conversation(
        ...
        if context.is_button_triggered:
            ...
            conversation_prompt = f"""
            The user clicked a suggestion button: "{intent}". This should be treated as an intention or interest — NOT as if the user has provided complete information.

            You must respond with a follow-up question that encourages the user to elaborate.

            - Do NOT thank the user
            - Do NOT say "noted", "understood", or anything that confirms completion
            - DO ask a natural, open-ended follow-up question

            Examples:
            - If the button was "Ngân sách dự kiến của bạn" → ask: "Bạn dự kiến ngân sách khoảng bao nhiêu cho dự án lần này?"
            - If the button was "Mục tiêu truyền thông" → ask: "Bạn có thể mô tả mục tiêu truyền thông cụ thể hơn không?"

            Respond in Vietnamese only. Return a valid JSON matching ConversationResponse.
            """
            fallback_response = {
                "message": "Bạn có thể chia sẻ thêm thông tin cụ thể hơn được không?",
                "state": context.state.value,
            ...
            result = await self.conversation_agent.arun(conversation_prompt)
            response = parse_agent_response(
                result, ConversationResponse, fallback_response
            )
            context.is_button_triggered = False
            return response
            ...
        try:
            ...
            if numbers and any(
                keyword in user_message.lower() for keyword in BUDGET_KEYWORDS
            ):
                if numbers:
            ...
            if detected_audiences:
                ...
                context.team_memory.workflow_plan["target_audience"] = (
                    detected_audiences
                )
            ...
            if any(
                ...
                for word in ["gấp", "urgent", "nhanh", "quick", "asap"]
            ):
                ...
                word in user_message.lower()
                for word in ["chậm", "slow", "từ từ", "không vội"]
            ):
            ...
            detected_goals = []
            # ✅ Chỉ phân tích nếu message đủ dài
            ...
            has_project_info = (
                bool(context.user_input or user_message) and len(user_message) > 20
            )
            has_budget = context.budget > 0
            has_basic_requirements = bool(
                context.requirements.get("industry")
                or context.requirements.get("objectives")
            )
            ...
            if (
                ...
                logger.info(
                    "State: GATHERING_INFO → ANALYZING (team ready for deployment)"
                )
            ...
            if len(user_message) > 20:
                if not context.user_input or len(user_message) > len(
                    context.user_input
                ):
            ...
            context.team_memory.context_summary = (
                self._extract_key_info_from_conversation(context)
            )
            context.team_memory.updated_at = datetime.utcnow()
            context.updated_at = datetime.utcnow()
    ...
    def _determine_media_categories(
        self, content_analysis: ContentAnalysis, context: ConversationContext
    ) -> List[str]:
        ...
        if (
            "Women" in content_analysis.target_audiences
            or "Families" in content_analysis.target_audiences
        ):
    ...
    def get_session_summary(self, session_id: str) -> Optional[Dict]:
        ...
        return {
            ...
            "has_results": bool(
                context.content_analysis or context.media_recommendations
            ),
        ...
        return {
            ...
            "team_metrics": {
                ...
                "average_confidence_score": sum(
                    context.team_memory.confidence_scores.values()
                )
                / max(len(context.team_memory.confidence_scores), 1),
                "error_rate": len(context.team_memory.error_log)
                / max(len(context.team_memory.completed_tasks), 1),
                "team_coordination_status": (
                    "successful"
                    if context.team_initialized and not context.team_memory.error_log
                    else "partial"
                ),
            },
            "orchestrator_model": (
                OPENAI_ORCHESTRATOR_MODEL if OPENAI_API_KEY else GROQ_ORCHESTRATOR_MODEL
            ),
            "worker_model": (
                OPENAI_WORKER_MODEL if OPENAI_API_KEY else GROQ_WORKER_MODEL
            ),
    ...
    async def coordinate_team_modification(
        ...
        try:
            ...
            if not target_specialists:
                ...
                target_specialists = specialist_mapping.get(
                    modification_type, ["ModificationSpecialist"]
                )
            ...
            logger.info(
                f"🔄 Coordinated {modification_type} modification team with {len(target_specialists)} specialists"
            )
    ...
    def health_check(self) -> Dict:
        ...
        try:
            ...
            try:
                ...
                for name, agent in core_agents:
                    agent_status[name] = {
                        ...
                        "has_tools": bool(
                            agent and hasattr(agent, "tools") and agent.tools
                        ),
                    }
            ...
            try:
                team_status = {}
                teams = [("AnalysisTeam", self.analysis_team), ("QATeam", self.qa_team)]
                ...
                for name, team in teams:
                    team_status[name] = {
                        ...
                        "coordination_mode": (
                            getattr(team, "mode", "unknown") if team else "unknown"
                        ),
                    }

                health_status["components"]["teams"] = team_status
            except Exception as e:
                health_status["components"]["teams"] = {
                    "status": "error",
                    "error": str(e),
                }
                health_status["system_status"] = "degraded"
            ...
            health_status["components"]["sessions"] = {
                ...
                "utilization": len(self.active_conversations)
            ...
            logger.info(
                f"🏥 Team system health check: {health_status['system_status']}"
            )
...
async def test_enhanced_team_system():
    ...
    try:
        ...
        if response2.can_proceed:
            ...
            if response3.data:
                logger.info(
                    f"✅ Team workflow data keys: {list(response3.data.keys())}"
                )
                ...
                if summary:
                    ...
                    logger.info(
                        f"  - Confidence: {summary.get('confidence_score', 0):.2f}"
                    )
                    logger.info(
                        f"  - Team Coordination: {summary.get('team_coordination')}"
                    )
        ...
        if metrics:
            ...
            logger.info(
                f"  - Agents Deployed: {team_metrics.get('total_agents_deployed', 0)}"
            )
            logger.info(
                f"  - Parallel Processing: {team_metrics.get('parallel_processing_enabled', False)}"
            )
            logger.info(
                f"  - Processing Time: {team_metrics.get('total_processing_time_seconds', 0):.1f}s"
            )
            logger.info(
                f"  - Coordination Status: {team_metrics.get('team_coordination_status')}"
            )
        ...
        modification_result = await enhanced_team_system.coordinate_team_modification(
            "test_enhanced_001", "budget", ["PricingSpecialist", "MediaSpecialist"]
        )
        logger.info(
            f"Modification Coordination: {modification_result.get('coordination_status')}"
        )
...
class ConversationalMediaReleaseAgents:
    ...
    async def continue_conversation(
        ...
        return await self.enhanced_system.continue_conversation(
            session_id, user_message, progress_callback
        )
    ...
    async def trigger_workflow(
        ...
        return await self.enhanced_system.trigger_workflow(
    ...
    async def modify_plan(
        ...
        return await self.enhanced_system.modify_plan(
            session_id, modification_request, progress_callback
        )
