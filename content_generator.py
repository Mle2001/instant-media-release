...
class ContentGeneratorToolkit(Toolkit):
    ...
    def generate_press_release(self, request: ContentRequest) -> GeneratedContent:
        ...
        try:
            ...
            template = self._get_content_template(request.content_type, request.industry)
    ...
    def optimize_seo(self, content: str, keywords: List[str]) -> str:
        ...
        for keyword in keywords:
            ...
            target_density = max(2, len(optimized_content.split()) // 100)  # 1-2% density
    ...
    def check_readability(self, content: str) -> Dict[str, Any]:
        ...
        readability_score = 100 - (avg_sentence_length * 1.015) - (complex_words * 84.6 / len(words) * 100)
    ...
    def create_content_variations(self, base_content: str, variations: int = 3) -> List[str]:
        """Tạo nhiều variations của content"""
        variations_list = []
        
        for i in range(variations):
            variation = self._create_variation(base_content, variation_type=f"style_{i+1}")
        ...
        for sentence in sentences:
            ...
            if len(sentence) > 50:  # Meaningful sentences
                ...
                if any(indicator in sentence.lower() for indicator in 
                      ['thông báo', 'ra mắt', 'phát triển', 'tăng', 'cải thiện', 'mới']):
    ...
    def _get_content_template(self, content_type: ContentType, industry: str) -> ContentTemplate:
        ...
        templates = {
            "TECHNOLOGY": ContentTemplate(
                ...
                required_fields=["company_name", "announcement_details", "target_audience"],
            ...
            "BUSINESS": ContentTemplate(
                ...
                required_fields=["company_name", "business_announcement", "market_impact"],
    ...
    def _create_content_structure(self, request: ContentRequest, template: ContentTemplate) -> List[str]:
        ...
        if request.length == ContentLength.SHORT:
            ...
            structure = [s for s in structure if s in ["headline", "lead_paragraph", "body_paragraph_1", "company_info"]]
        elif request.length == ContentLength.EXTENDED:
            # Add more sections for extended content
            structure.extend(["additional_details", "market_analysis", "technical_specs"])
        
        return structure
    
    def _generate_section_content(self, section: str, request: ContentRequest, template: ContentTemplate) -> str:
    ...
    def _generate_headline(self, section: str, request: ContentRequest, template: ContentTemplate) -> str:
        """Tạo headline"""
    ...
    def _generate_dateline(self, section: str, request: ContentRequest, template: ContentTemplate) -> str:
        """Tạo dateline"""
        return f"TP. Hồ Chí Minh, {datetime.now().strftime('%d/%m/%Y')}"
    
    def _generate_lead_paragraph(self, section: str, request: ContentRequest, template: ContentTemplate) -> str:
        """Tạo lead paragraph"""
        company = request.company_name
        announcement = request.announcement_details.get("description", "phát triển mới")
        benefit = request.announcement_details.get("benefit", "mang lại giá trị cho khách hàng")
    ...
    def _generate_body_paragraph(self, section: str, request: ContentRequest, template: ContentTemplate) -> str:
        """Tạo body paragraph"""
        details = request.announcement_details
        
        features = details.get("features", ["tính năng tiên tiến", "giao diện thân thiện", "bảo mật cao"])
    ...
    def _generate_quote(self, section: str, request: ContentRequest, template: ContentTemplate) -> str:
        """Tạo quote"""
        if not request.include_quotes:
            return ""
        
        executive = request.announcement_details.get("executive", "CEO")
        key_message = request.key_messages[0] if request.key_messages else "Đây là một cột mốc quan trọng cho chúng tôi"
    ...
    def _generate_company_info(self, section: str, request: ContentRequest, template: ContentTemplate) -> str:
        """Tạo company info"""
        return f"Về {request.company_name}: {request.company_description}"
    
    def _generate_contact_info(self, section: str, request: ContentRequest, template: ContentTemplate) -> str:
        """Tạo contact info"""
...
Điện thoại: +84 xxx xxx xxxx"""
    ...
    def _generate_generic_section(self, section: str, request: ContentRequest, template: ContentTemplate) -> str:
        """Generic section generator"""
        return f"[Nội dung cho section: {section}]"
    
    def _combine_sections(self, sections: Dict[str, str], request: ContentRequest) -> str:
    ...
    def _save_generated_content(self, content: GeneratedContent):
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                content.content_id, content.request_id, content.order_id,
                content.title, content.subtitle, content.content, content.summary,
                content.meta_description, json.dumps(content.tags), content.word_count,
                content.seo_score, content.readability_score,
                content.created_at.isoformat(), content.version
            ))
...
class ContentGeneratorAgent(Agent):
    ...
    def create_press_release(self, order_id: str, company_info: Dict[str, Any],
                            announcement: Dict[str, Any], preferences: Dict[str, Any] = None) -> GeneratedContent:
    ...
    def enhance_existing_content(self, content_id: str, enhancement_types: List[str]) -> GeneratedContent:
        ...
        for enhancement_type in enhancement_types:
            enhanced_content = self.content_toolkit.enhance_content(enhanced_content, enhancement_type)
        
        # Recalculate metrics
        word_count = len(enhanced_content.split())
        seo_score = self.content_toolkit._calculate_seo_score(enhanced_content, [])
        readability_score = self.content_toolkit._calculate_readability_score(enhanced_content)
    ...
    def _save_content_request(self, request: ContentRequest):
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                request.request_id, request.order_id, request.content_type.value,
                request.industry, request.company_name, request.company_description,
                json.dumps(request.announcement_details), request.target_audience,
                request.tone.value, request.length.value,
                json.dumps(request.key_messages), json.dumps(request.seo_keywords),
                request.include_quotes, request.include_statistics,
                request.language, request.created_at.isoformat()
            ))
